...
const CATS: { key: string; label: string; description: string; type: NotificationType; level: number; icon: string; href?: string; broadcastable?: boolean }[] = [
  ...
  { key: 'chat', label: 'Mensajes de chat', description: 'Mensajes entre pacientes y el equipo de salud.', type: NotificationType.SYSTEM, level: 2, icon: '💬', href: '/mensajes-chat' },
